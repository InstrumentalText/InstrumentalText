...
5. If the user's intent cannot be fulfilled by any available action, output: {""error"": true, ""reason"": ""<explanation>""}";
    ...
    public void ProcessPrompt(GameObject obj, string userIntent)
    {
        ...
        {
            Debug.LogWarning("[LLMProcessorOnDevice] No handlers found on target object.");
            return;
        }
    ...
    private IEnumerator SendLLMRequest(GameObject obj, string userMessage)
    {
        ...
        {
            Debug.LogError($"[LLMProcessorOnDevice] API request failed: {request.error}");
        ...
        Debug.Log($"[LLMProcessorOnDevice] Raw response: {responseText}");
        ...
        {
            Debug.LogError($"[LLMProcessorOnDevice] Failed to parse response: {e.Message}");
            yield break;
        }

        if (string.IsNullOrEmpty(actionJson))
        {
            Debug.LogError("[LLMProcessorOnDevice] Empty response from LLM.");
            yield break;
        }

        actionJson = ExtractJson(actionJson);

        Debug.Log($"[LLMProcessorOnDevice] Action JSON: {actionJson}");
        ...
        try
        {
            ...
            if (parsed["error"] != null)
            {
                ...
                Debug.LogWarning($"[LLMProcessorOnDevice] Planner error: {reason}");
                yield break;
            }
        }
        catch (Exception e)
        {
            Debug.LogError($"[LLMProcessorOnDevice] Invalid JSON: {actionJson}");
